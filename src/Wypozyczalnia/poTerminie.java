...
public class poTerminie extends javax.swing.JFrame {
public DefaultTableModel model;
static JFrame wypozyczalnia;
    public poTerminie() throws SQLException {
    ...
    DefaultTableModel model2 = (DefaultTableModel) jTable3.getModel();
        try (Connection Con = DB.getConnection()) {
            PreparedStatement ps = Con.prepareStatement("SELECT samochody.ID_auta, samochody.Nr_rejestracyjny, samochody.`Marka`, samochody.`Model`, samochody.Data_konca_ubezpieczenia FROM samochody WHERE DATEDIFF(CURRENT_DATE(),`Data_konca_ubezpieczenia`) >0;", ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
         ...
         try (Connection Con = DB.getConnection()) {
            PreparedStatement ps = Con.prepareStatement("SELECT samochody.ID_auta, samochody.Nr_rejestracyjny, samochody.`Marka`, samochody.`Model`, samochody.`Data_waznosci_przegladu` FROM samochody WHERE DATEDIFF(CURRENT_DATE(),`Data_waznosci_przegladu`) >0;", ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
            ResultSet rs = ps.executeQuery();
...
public void openFile(String file) {
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                try {
                    wypozyczalnia = new poTerminie();
                    wypozyczalnia.setLocationRelativeTo(null);
                    wypozyczalnia.setVisible(true);
                    
                } catch (SQLException ex) {
                    Logger.getLogger(poTerminie.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
        });
    }
